...
### 理解DQN的局限性  
即使是深度Q-learning也存在一些限制，无论您是通过视觉图像或其他对于环境的观察来近似q函数：
...
* Q-learning的机制导致的另一个结果是，它并不是从通用的角度来理解游戏的进行，而是从一个特定的角度来理解(它重复了它在训练中所体验到的有效的东西)。因此，任何引入游戏的新颖事物(在训练过程中从未体验过)都会破坏算法，使其完全无效。在算法中引入新游戏时也是如此，它根本无法执行。  
